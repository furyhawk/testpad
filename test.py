...
def main(words, word_len, num_answers=20):
  ...
  for i in range(word_len):
    for j in range(word_len):
      ...
      model.Add(
          model.AddElement(E[j] * word_len + i, E[i] * word_len + j, j))
      # solver.Add(
  ...
  solution_printer = VarArraySolutionPrinter(E, words)
...
def read_words(word_list, word_len, limit):
  ...
  for w in words:
    w = w.strip().lower()
    # if len(w) == word_len and not dict.has_key(w) and not re.search("[^a-z???]",w) and count < limit:
    # Later note: The limit is not needed anymore with Mistral
    if len(w) == word_len and w not in dict and not re.search("[^a-z???]", w):
      dict[w] = 1
      all_words.append(w)
      count += 1
  return all_words
